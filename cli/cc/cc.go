...
var (
	ErrNotRunning = errors.New("eamerald is not running, use 'eamerald start' or 'eamerald run' to start")
	ErrIsRunning  = errors.New("eamerald is already running, use 'eamerald stop' to stop")
)
...
func NewConfig(ctx context.Context, noCheck bool, configFilePath string) (*Config, error) {
	cfg := &Config{
		Version: 1,
		Active: ActiveConfig{
			ConfigFile: filepath.Join(GetEameraldCfgDir(), "config.yaml"),
			Config:     "config.yaml",
		},
		Defaults: DefaultsConfig{
			NoCheck: noCheck,
...
func (cfg *Config) SaveContextConfig(configurationFile string) error {
	cliConfig := filepath.Join(GetEameraldDir(), configurationFile)
