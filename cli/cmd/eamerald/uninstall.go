...
func (cmd *UninstallCmd) Run(ctx context.Context) error {
	...
	if err := removeFile(filepath.Join(cc.GetEameraldDataDir(), "directory.db")); err != nil {
		return err
	}
	// remove directory.db.sync file.
	if err := removeFile(filepath.Join(cc.GetEameraldDataDir(), "directory.db.sync")); err != nil {
		return err
	}
	// remove config.yaml file last.
	if err := removeFile(filepath.Join(cc.GetEameraldCfgDir(), "config.yaml")); err != nil {
		return err
	}
