...
func (cmd *InstallTemplateCmd) Run(ctx context.Context, cfg *cc.Config) error {
	...
	cfg.Active.ConfigFile = filepath.Join(cc.GetEameraldCfgDir(), fileName)
	...
	if _, err := os.Stat(cc.GetEameraldDir()); os.IsNotExist(err) {
		if err := os.MkdirAll(cc.GetEameraldDir(), fs.FileModeOwnerRWX); err != nil {
			return err
		}
	}

	cliConfig := filepath.Join(cc.GetEameraldDir(), common.CLIConfigurationFile)
